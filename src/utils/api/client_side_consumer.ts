...
export function usePonto(id_ponto: number) {
  ...
  useEffect(() => {
    consumerPonto.get(id_ponto.toString()).then((data) => setPonto(data));
  }, [id_ponto]);
  return ponto;
}
...
export function useReservatorio(id_reservatorio: number) {
  ...
  useEffect(() => {
    consumerReservatorio
      .get(id_reservatorio.toString())
...
export function usePontos(
  codigo_edificacao: string | null = null,
  id_sequencia: number | null = null,
) {
...
export async function delPonto(id_ponto: number) {
  ...
  if (response.status === 200) {
    ...
    alert(
      'Ponto não pode ser deletado pois possui coletas ou sequência de coleta associadas!',
    );
  }
...
export function useSequencias() {
  ...
  useEffect(() => {
    consumerSequencia.list().then((data) => setSequencias(data));
  }, []);
...
export function useColeta(id_coleta: number) {
  ...
  useEffect(() => {
    consumerColeta.get(id_coleta.toString()).then((data) => setColeta(data));
  }, [id_coleta]);
...
export function useColetas() {
  ...
  useEffect(() => {
    consumerColeta.list().then((data) => setColetas(data));
  }, []);
...
export function useUsuario(username: string) {
  ...
  useEffect(() => {
    consumerUsuario.get(username).then((data) => setUsuario(data));
  }, [username]);
