      ...
      {currentPontoId && coleta && (
        ...
        >
          ...
          <select
            ...
            onChange={(e) =>
              setCurrentSequenciaId(parseInt(e.currentTarget.value))
            }
            required
          >
            {sequencias.map((sequencia: Sequencia) => {
              return (
                <option value={sequencia.id} key={sequencia.id}>
                  {sequencia.id} - Ciclo de Amostragem {sequencia.amostragem} -
                  Ponto {TIPOS_PONTOS[sequencia.ponto?.tipo || 0]}
                </option>
              );
            })}
          </select>
          ...
          <div className="flex">
            <select
              ...
              onChange={(e) =>
                setCurrentPontoId(parseInt(e.currentTarget.value))
              }
            >
              {pontos.map((ponto) => (
                <option value={ponto.id} key={'ponto ' + ponto.id}>
            ...
            <a
              ...
              href={currentPontoId ? '/admin/pontos/' + currentPontoId : '#'}
              target={currentPontoId ? '_blank' : '_self'}
            >
              <svg
                className={
                  currentPontoId
          ...
          <div className="flex gap-2">
            <input
              ...
              id="escherichia"
              name="escherichia"
              className="h-6 w-6 rounded-lg border border-neutral-200 px-6 py-4 disabled:bg-neutral-200 disabled:text-neutral-500"
              disabled={!editable}
          ...
          <div className="flex gap-4">
            <label htmlFor="data" className="flex self-center">
              Data e hora:
            </label>
            <input
              type="date"
              id="data"
              name="data"
              className="flex-grow rounded-lg border border-neutral-200 px-6 py-4 disabled:bg-neutral-200 disabled:text-neutral-500   "
              disabled={!editable}
            ...
            {responsaveis.map((usuario) => {
              return (
                <option
                  ...
                  selected={
                    coleta.responsaveis_id &&
                    coleta.responsaveis_id.includes(usuario.id)
                  }
                >
                  {usuario.username}
                </option>
