...
export function useSearchFilter() {
  ...
  const handleSearch = useDebouncedCallback(async (query: string) => {
    await setSearch(query || null); // Remove o parâmetro da URL se vazio
  }, 300);

  const handleCampusChange = async (selected: string) => {
    await setCampus(selected === DEFAULT_CAMPUS_VALUE ? null : selected); // Remove o parâmetro da URL se for o valor padrão
  };
