...
function PointDashboard(props: {coleta: Coleta}) {
    ...
    return (
        <div className="flex gap-4 flex-wrap">
            <div className={"w-2/5 aspect-square rounded-lg p-6 shadow " + bg_color}>
...
export default async function Page(props: { 
    ...
    const { params } = props;
    var resp = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/pontos/${params.id_ponto}`);
    var ponto: Ponto = await resp.json();
    
    resp = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/pontos/${params.id_ponto}/coletas`);
